  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const usersWithSessions = users.map(user => {
        const activeSession = activeSessions.find(s => s.user_id === user.id);
        return { ...user, activeSession: activeSession || null };
      });
...
export const updateUser = createAsyncThunk(
  ...
  async ({ userId, login, name, phone }, { rejectWithValue, dispatch }) => {
    try {
      await apiClient.patch(`/users?id=eq.${userId}`, { login, name, phone });
      dispatch(fetchUsers());
      return { success: true };
    } catch (error) {
...
export const topUpBalance = createAsyncThunk(
  ...
  async ({ userId, amount, paymentMethodId }, { rejectWithValue, dispatch }) => {
    try {
      ...
      await apiClient.patch(`/users?id=eq.${userId}`, { balance: newBalance });
...
const usersSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(topUpBalance.rejected, (state, action) => { state.error = action.payload; });
  },
});
